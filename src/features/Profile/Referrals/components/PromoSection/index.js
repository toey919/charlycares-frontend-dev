...
const PromoContainer = ({ referralSettings }) => {
  if (!referralSettings) return null;
  const { message, link, content, title, mail_subject } = referralSettings;
  ...
  return (
    <Container>
      ...
      <ShareHeading>
        <FormattedMessage id="profile.referrals.shareWithFriends" />
      </ShareHeading>
      <ButtonsContainer>
        <ButtonWrapper>
          {isMobile ? (
